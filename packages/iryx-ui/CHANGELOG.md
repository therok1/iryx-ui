...
- 4449be8: Fix five WCAG contrast failures found by sweeping every component for alpha-dimmed text and `currentColor` fills, the pattern behind the earlier `IKbd` bug. Measured against a muted context in light mode, which is the binding case:
...
- fc23400: Add `ICarousel` — a row of slides you page through, with arrows, dots and the
  arrow keys.
